...
public class ServicePingProcessor(PerRequestContext perRequestContext,
    ...
    IHttpClientFactory httpClientFactory)
{
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        var daqs = await dbManagerFactory.ServiceManager.GetAsync(false, false, cancellationToken).ConfigureAwait(false);
        foreach (var daq in daqs.Items)
        {
            ...
            try
            {
                ...
                await dbManagerFactory.ServiceManager.HandlePingStatusAsync(daq.Id, response, cancellationToken).ConfigureAwait(false);
            }
            catch (Exception e)
            {
